...
async function fetchKnabenData(searchQuery, type = 'movie') {
    ...
    console.log(`🦉 [HTML Scraper] Searching Knaben.org for: "${cleanedQuery}" (Pages 1-3)`);

    const allStreams = [];
    const maxPages = 3; // Cycle through first 3 pages

    for (let page = 1; page <= maxPages; page++) {
        try {
            // URL format: /search/query/0/page_number
            // Note: Knaben uses 0 for category (all) and page number starts at 1? Or 0?
            // Based on user request: /search/query/0/1, /search/query/0/2
            const searchUrl = `${KNABEN_BASE_URL}/search/${encodeURIComponent(cleanedQuery)}/0/${page}/`;
            
            console.log(`🦉 [Page ${page}] Fetching: ${searchUrl}`);
            ...
            console.log(`🦉 [Page ${page}] Found ${rows.length} potential results.`);

            const pageStreams = rows.toArray().map(row => {
                const $row = $(row);
            ...
            console.error(`❌ Error in fetchKnabenData (Page ${page}):`, error.message);
        }
    }

    console.log(`🦉 Successfully parsed ${allStreams.length} total streams from Knaben.org (Pages 1-${maxPages}).`);
    return allStreams;
}
...
async function handleStream(type, id, config, workerOrigin) {
    ...
    try {
        ...
        for (const query of finalSearchQueries) {
            ...
            if (useKnaben) {
                // 🔥 MODIFIED: Always search Knaben, even for "Stagione" or "Completa"
                // The user wants to ensure we catch everything, even if it seems Italian-specific
                searchPromises.push({
                    name: 'Knaben',
